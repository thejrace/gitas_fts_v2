...
public class Excel_IYS_Rapor {

    private String tarih;
    private Refresh_Listener listener;
    public Excel_IYS_Rapor( String _tarih ){
        tarih = _tarih;
    }
    ...
    public void init(){
        ...
        Thread th = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    try {
                        Web_Request request = new Web_Request(Web_Request.SERVIS_URL, "&req=iys_excel_veri_download&baslangic="+tarih+"&bitis=" );
                        request.kullanici_pc_parametreleri_ekle();
                        request.action();
                        JSONArray data = new JSONObject(request.get_value()).getJSONObject("data").getJSONArray("iys_data");

                        int row = 1;

                        if ( data.length() == 0 ) return;

                        myFirstWbook = Workbook.createWorkbook(new File("C:\\temp\\IYS_Rapor_" + tarih + ".xls"));
                        WritableSheet excelSheet = myFirstWbook.createSheet("IYS Rapor", 0);
                        ...
                        label = new Label(6, 0, "TİP", cFormat);
                        excelSheet.addCell(label);

                        label = new Label(7, 0, "ÖZET", cFormat);
                        excelSheet.addCell(label);
                        ...
                        for (int j = 0; j < data.length(); j++) {
                            ...
                            for( int k = 0; k < otobus_array.length(); k++ ){
                                ...
                                label = new Label(6, row, res.getString("tip"), aktif_stil);
                                excelSheet.addCell(label);

                                label = new Label(7, row, res.getString("ozet"), aktif_stil);
                                excelSheet.addCell(label);
